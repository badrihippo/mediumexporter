...
async function downloadImages(images, options = {}) {
  ...
  // ensure featured image is on list
  if (!!options.featuredImage && !!options.featuredImageWidth) {
    options.featuredImageUrl =
      MEDIUM_IMG_CDN +
      Math.max(options.featuredImageWidth * 2, 2000) +
      '/' +
      options.featuredImage

    //check if it's there
    if (images.indexOf(options.featuredImageUrl) == -1) {
      images.push(options.featuredImageUrl)
    }
  }
